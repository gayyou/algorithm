...
class SuffixArray {
  ...
  public static void createSuffixArray(String str, int[] sa, int[] LCP) {
    ...
    for (int i = 0; i < N; i++)
      s[i] = str.charAt(i);  // 将字符串变成对应的ascII
  ...
  public static void makeSuffixArray(int[] s, int[] SA, int n, int K) {
    ...
    int n1 = (n + 1) / 3;
    int n2 = n / 3;
    int t = n0 - n1;  // 1 iff n%3 == 1   1或者0
    int n12 = n1 + n2 + t;

    int[] s12 = new int[n12 + 3];  // 根据长度进行创建数组，创建的长度为  2/3 或者 2/3 + 2
    int[] SA12 = new int[n12 + 3];  // 根据长度进行创建数组，创建的长度为  2/3 或者 2/3 + 2
    int[] s0 = new int[n0];  // 创建数组
    int[] SA0 = new int[n0];
  ...
  // int[] offset = 1;
  // int[] s = new int[]{0, 1, 2, 3, 4, 5, 6}
  // count = {0, 0, 1, 1, 1, 1, 1, 1}
  // count = {0, 0, 1, 2, 3, 4, 5, 6}
  // count = {0, 0, 2, 3, 4, 5, 6, 7}
  // out = {0, 0, 0, 1, 2, 3, 4, 5}
  private static void radixPass(int[] in, int[] out, int[] s, int offset,
    ...
    for (int i = 0; i < n; i++)
      count[s[in[i] + offset] + 1]++;    // count occurences  // 统计出s[in[i] + offset] + 1 位置的个数

    for (int i = 1; i <= K + 1; i++)            // compute exclusive sums
      count[i] += count[i - 1];     // 将数字向后 + 1
