...
public class Jipa {
  ...
  public static void processInstruction(String instruction) {
    ...
    if (spaceLoc != -1) {  // 当有空格的时候进行去除空格及之前的东西， TODO 这里是执行函数的时候代码
      ...
      operand = splitArguments(instruction.substring(spaceLoc + 1));  // 获取函数的所有参数并且返回参数列表
    } else {
      // No arguments
      opcode = instruction;
